  ...
  return (
    <>
          ...
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            >
              ...
              <motion.p
                ...
                className="text-xl md:text-5xl font-semibold text-neutral-700 mt-4 dark:text-white"
              >
                {card.title}
              </motion.p>
              <div className="pb-10 pt-4">{card.content}</div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-xl h-80 w-56 md:h-[40rem] md:w-96 overflow-hidden flex flex-col items-start justify-start relative z-10"
      >
...
export const BlurImage = ({ src, alt, className, ...rest }: ImageProps) => {
  ...
  return (
    <img
      className={`${isLoading ? "blur-sm" : "blur-0"} ${className} object-cover h-full w-full transition duration-300`}
